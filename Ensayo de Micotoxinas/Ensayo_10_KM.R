...
shapiro_test_controlSac <- shapiro.test(df_ensayo_10$controlSac)
shapiro_test_controlPGBQ <- shapiro.test(df_ensayo_10$controlPGBQ)
shapiro_test_BbQ150 <- shapiro.test(df_ensayo_10$BbQ150)
shapiro_test_BbQ300 <- shapiro.test(df_ensayo_10$BbQ300)
shapiro_test_BbQ450 <- shapiro.test(df_ensayo_10$BbQ450)
shapiro_test_BbQ600 <- shapiro.test(df_ensayo_10$BbQ600)
shapiro_test_BbQ750 <- shapiro.test(df_ensayo_10$BbQ750)
...
levene_test <- leveneTest(
  c(controlSac = df_ensayo_10$controlSac,
    controlPGBQ = df_ensayo_10$controlPGBQ,
    BbQ150 = df_ensayo_10$BbQ150,
    BbQ300 = df_ensayo_10$BbQ300,
    BbQ450 = df_ensayo_10$BbQ450,
    BbQ600 = df_ensayo_10$BbQ600,
    BbQ750 = df_ensayo_10$BbQ750) ~ 
    factor(rep(c("controlSac", "controlPGBQ","BbQ150", "BbQ300", "BbQ450", "BbQ600", "BbQ750"),
               each = nrow(df_ensayo_10))),
  center = median
)

levene_test_2 <- leveneTest(
  c(BbQ150 = df_ensayo_10$BbQ150,
    BbQ300 = df_ensayo_10$BbQ300,
    BbQ450 = df_ensayo_10$BbQ450,
    BbQ600 = df_ensayo_10$BbQ600,
    BbQ750 = df_ensayo_10$BbQ750) ~ 
...
ensayo_10_controlSac.km <- survfit(Surv(tiempoControlSac,controlSac) ~ 1, data = df_ensayo_10, type = "kaplan-meier")
ensayo_10_controlPGBQ.km  <- survfit(Surv(tiempoControlPGBQ,controlPGBQ) ~ 1, data = df_ensayo_10, type = "kaplan-meier")
ensayo_10_BbQ150.km <- survfit(Surv(tiempoBbQ150,BbQ150) ~ 1, data = df_ensayo_10, type = "kaplan-meier")
ensayo_10_BbQ300.km <- survfit(Surv(tiempoBbQ300,BbQ300) ~ 1, data = df_ensayo_10, type = "kaplan-meier")
ensayo_10_BbQ450.km <- survfit(Surv(tiempoBbQ450,BbQ450) ~ 1, data = df_ensayo_10, type = "kaplan-meier")
ensayo_10_BbQ600.km <- survfit(Surv(tiempoBbQ600,BbQ600) ~ 1, data = df_ensayo_10, type = "kaplan-meier")
ensayo_10_BbQ750.km <- survfit(Surv(tiempoBbQ750,BbQ750) ~ 1, data = df_ensayo_10, type = "kaplan-meier")

ensayo_10_survfits <- list(controlSac = ensayo_10_controlSac.km,
                          controlPGBQ = ensayo_10_controlPGBQ.km,
                          BbQ150 = ensayo_10_BbQ150.km,
                          BbQ300 = ensayo_10_BbQ300.km,
                          BbQ450 = ensayo_10_BbQ450.km,
                          BbQ600 = ensayo_10_BbQ600.km,
                          BbQ750 = ensayo_10_BbQ750.km)

survfits <- list(
  "Control Sac" = survfit(Surv(tiempoControlSac,controlSac) ~ 1, data = df_ensayo_10, type = "kaplan-meier", conf.type = "log-log"),
  "Control PGQ" = survfit(Surv(tiempoControlPGBQ,controlPGBQ) ~ 1, data = df_ensayo_10, type = "kaplan-meier", conf.type = "log-log"),
  "BbQ 150 µL" = survfit(Surv(tiempoBbQ150,BbQ150) ~ 1, data = df_ensayo_10, type = "kaplan-meier", conf.type = "log-log"),
  "BbQ 300 µL" = survfit(Surv(tiempoBbQ300,BbQ300) ~ 1, data = df_ensayo_10, type = "kaplan-meier", conf.type = "log-log"),
  "BbQ 450 µL" = survfit(Surv(tiempoBbQ450,BbQ450) ~ 1, data = df_ensayo_10, type = "kaplan-meier", conf.type = "log-log"),
  "BbQ 600 µL" = survfit(Surv(tiempoBbQ600,BbQ600) ~ 1, data = df_ensayo_10, type = "kaplan-meier", conf.type = "log-log"),
  "BbQ 750 µL" = survfit(Surv(tiempoBbQ750,BbQ750) ~ 1, data = df_ensayo_10, type = "kaplan-meier", conf.type = "log-log")
)
...
grafico <- ggplot(df_surv, aes(x = time, y = surv, color = strata, linetype = strata, shape = strata)) +
  ...
  labs(
    title = "Curva de Supervivencia - Ensayo #10",
    x = "Tiempo (días)",
...
print(summary(ensayo_10_controlSac.km))
print(summary(ensayo_10_controlPGBQ.km))
print(summary(ensayo_10_BbQ300.km))
print(summary(ensayo_10_BbQ450.km))
print(summary(ensayo_10_BbQ600.km ))
print(summary(ensayo_10_BbQ750.km ))

print(summary(ensayo_10_controlSac.km)$table["median"])
print(summary(ensayo_10_controlPGBQ.km)$table["median"])
print(summary(ensayo_10_BbQ300.km)$table["median"])
print(summary(ensayo_10_BbQ450.km)$table["median"])
print(summary(ensayo_10_BbQ600.km)$table["median"])
print(summary(ensayo_10_BbQ750.km)$table["median"])

#Test de Cox-Matlan

df_controlSac <- data.frame(tiempo = ensayo_10$tiempo_dias,
                            muertas = ensayo_10$X.Muertas,
                            tratamiento = "ControlSac")

df_controlPGBQ <- data.frame(tiempo = ensayo_10$tiempo_dias.1,
                             muertas = ensayo_10$X.Muertas.1,
                             tratamiento = "ControlPGBQ")

df_Bb150 <- data.frame(tiempo = ensayo_10$tiempo_dias.2,
                       muertas = ensayo_10$X.Muertas.2,
                       tratamiento = "Bb150")

df_Bb300 <- data.frame(tiempo = ensayo_10$tiempo_dias.3,
                       muertas = ensayo_10$X.Muertas.3,
                       tratamiento = "Bb300")

df_Bb450 <- data.frame(tiempo = ensayo_10$tiempo_dias.4,
                       muertas = ensayo_10$X.Muertas.4,
                       tratamiento = "Bb450")

df_Bb600 <- data.frame(tiempo = ensayo_10$tiempo_dias.5,
                       muertas = ensayo_10$X.Muertas.5,
                       tratamiento = "Bb650")

df_Bb750 <- data.frame(tiempo = ensayo_10$tiempo_dias.6,
                       muertas = ensayo_10$X.Muertas.6,
                       tratamiento = "Bb750")
...
#png("Ensayo-10-Supervivencia.png",width = 5, height = 5, units = 'in', res = 600,pointsize=7.5)
#par(xpd=T, mar=par()$mar+c(0,0,0,10))
#plot(ensayo_10_controlSac.km, xlab="Tiempo (días)", ylab="Supervivencia = S(t)", main="Curva de Supervivencia - Ensayo #10", conf.int = F, col = "seagreen1", lwd=2, mark.time = TRUE, las = 1)
#lines(ensayo_10_controlPGBQ.km, conf.int = F, col = "darkblue" , lwd = 2, mark.time = TRUE)
#lines(ensayo_10_BbQ150.km, conf.int = F, col = "violet" , lwd = 2, mark.time = TRUE)
#lines(ensayo_10_BbQ300.km, conf.int = F, col = "darkolivegreen3" , lwd = 2, mark.time = TRUE)
#lines(ensayo_10_BbQ450.km, conf.int = F, col = "coral2" , lwd = 2, mark.time = TRUE)
#lines(ensayo_10_BbQ600.km, conf.int = F, col = "chocolate3" , lwd = 2, mark.time = TRUE)
#lines(ensayo_10_BbQ750.km, conf.int = F, col = "deeppink3" , lwd = 2, mark.time = TRUE)
